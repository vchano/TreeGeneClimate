...
#SBATCH -N 1
#SBATC --exclusive
#SBATCH --job-name=WES.COV
#SBATCH --output=BED.WES.cov_%J_out.txt
#SBATCH --error=BED.WES.cov_%J_err.txt
...
module purge # to get rid of previously loaded modules that could interfere
module load bedtools/2.18.0 # Newer versions of bedtools don't work properly when inputing BAM or BED files -abam or -b 

# Set PATHS to working directories

INPUT=$SCRATCH/TGC_WES/3.MAPPING.BOWTIE2
OUTPUT=$SCRATCH/TGC_WES/4.COVERAGE
REF=$SCRATCH/REF_GENOME/TGC_probes_design # We need the BED file for the probe sites (-b parameter)

# NOTE: 
# Running coverage from BEDTOOLS v2.18.0 on bam files from BOWTIE2 to obtain the coverage across the probe regions and the targeted genes in bed file
# There is a problem with more recent versions of Bedtools (from 2.24), where you need to change -abam by -b for bam files and -b by -a for bed files
# Samtools depth also works for this (see Mengmeng Lu code in Github), but I will use bedtools coverage from version bedtools/2.18.0 in HPC
# I will also use the code from https://gettinggeneticsdone.blogspot.com/2014/03/visualize-coverage-exome-targeted-ngs-bedtools.html, with grep for "all"
...
#-------------------------------------------------------------------------------
# Excute this script as sbatch $SCRATCH/TGC_WES/0.APPS/4.tgc.wes.bedtools.coverage.sh
#-------------------------------------------------------------------------------
